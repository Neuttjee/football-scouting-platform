          ...
          players.map((p) => {
            ...
            return (
              <TableRow key={p.id}>
                ...
                <TableCell className={contractExpired ? "text-text-muted" : ""}>
                  {p.teamLabel ?? "-"}
                </TableCell>
                ...
                {/* Bij club sinds */}
                <TableCell className={contractExpired ? "text-text-muted" : ""}>
                  {p.joinedAt ? new Date(p.joinedAt).toLocaleDateString("nl-NL") : "-"}
                </TableCell>

                {/* Contract tot */}
                <TableCell className={contractExpired ? "text-text-muted" : ""}>
                  {p.contractEndDate
                    ? new Date(p.contractEndDate).toLocaleDateString("nl-NL")
                    : "-"}
                </TableCell>
              </TableRow>
            );
